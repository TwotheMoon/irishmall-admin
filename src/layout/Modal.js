...
export const CommonModal = ({ inputValues, onInputChange, onConfirm }) => {
  ...
  return(
    ...
    >
      <CModalHeader><CModalTitle>{showModal?.title}</CModalTitle></CModalHeader>
      ...
      <CModalFooter>
        {showModal.isCancelVisible ?
          <CButton color="secondary" onClick={() => commonCloseModal(setShowModal)}>취소</CButton>
          :
          null
        }
        <CButton color="primary" onClick={modalType === "default" || modalType === "error" ? showModal.onClick : () => onConfirm()}>확인</CButton>
      </CModalFooter>
    </CModal>
  )
}
