...
const SearchKeywordToolTab = () => {
  ...
  const checkKeywordCount = () => {
    if (!memoTxAreaRef.current.value || memoTxAreaRef.current.value.trim().length === 0) {
      setKeywordCount(0);
      return;
    }
    try {
...
export default SearchKeywordToolTab