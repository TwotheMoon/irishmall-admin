...
const NasLog = () => {
  ...
  const getNasLog = async () => {
    try {
      const res = await axios.get(`${isLocal ? localServerBaseUrl : apiServerBaseUrl}${getNasLogApiEP}`);

      setLogs(
        {
          connectionLogs: res?.data[0].series[0].values.map((list) => ({
            timestamp: formattedUnixToDate(list[0]),
            log: list[2]
          })).reverse(),
          systemLogs: res?.data[0].series[1].values.map((list) => ({
            timestamp: formattedUnixToDate(list[0]),
            log: list[2]
          })).reverse(),
        }
      ...
      <CAccordionItem itemKey={1} className='mb-4'>
        ...
        <CAccordionBody>
          <div className="fw-bolder">
            ...
            <CVirtualScroller className="border p-3" visibleItems={20} onScroll={(currentItemIndex) => setIndex(currentItemIndex)}>
              {logs.connectionLogs.map((log, i) => (
                <div key={i} className="d-flex align-item-center gap-3">
                  <div className="fw-bolder" style={{fontSize: "14px"}}>{log.timestamp}</div> 
